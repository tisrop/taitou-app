...
/// 用户主页「已解决」tab:被采纳的回答列表(offset 分页)。
///
/// 从 _UserProfilePageState 的 _solved*/_loadSolved/_buildSolvedList 抽出。
class SolvedTab extends ConsumerStatefulWidget {
  const SolvedTab({super.key, required this.username});

  final String username;

  @override
  ConsumerState<SolvedTab> createState() => _SolvedTabState();
}

class _SolvedTabState extends ConsumerState<SolvedTab> {
  static final _paginationHelper = PaginationHelpers.forList<SolvedPost>(
    keyExtractor: (p) => p.postId,
    expectedPageSize: 30,
  );

  final LoadMoreCoordinator _loadMoreCoordinator = LoadMoreCoordinator();
  List<SolvedPost>? _cache;
  bool _hasMore = true;
  bool _loading = true;
  ...
  Future<void> _load({bool loadMore = false}) async {
    ...
    try {
      ...
      final offset = loadMore ? (_cache?.length ?? 0) : 0;
      final response = await service.getUserSolvedPosts(widget.username, offset: offset);

      if (mounted) {
        setState(() {
          if (loadMore) {
            final currentState = PaginationState<SolvedPost>(items: _cache ?? []);
      ...
      child: M3eRefreshIndicator(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return UserProfileItems.solvedItem(context, posts[index]);
          },
        ),
      ),
    );
